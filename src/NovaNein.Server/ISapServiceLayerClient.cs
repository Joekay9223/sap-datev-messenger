...
public interface ISapServiceLayerClient
{
	...
	Task AttachPdfAsync(SapDocumentKind kind, int docEntry, int expectedDocNum, string localPdfPath, CancellationToken cancellationToken = default(CancellationToken));
	...
	{
		return Task.FromResult(new SapAccountValidation(account, false, false, false, null, "SAP-Kontenprüfung ist nicht verfügbar."));
	}

	Task<SapTaxCodeValidation> ValidateTaxCodeAsync(string taxCode, CancellationToken cancellationToken = default)
	{
		return Task.FromResult(new SapTaxCodeValidation(taxCode, false, false, null, "SAP-Steuerschlüsselprüfung ist nicht verfügbar."));
	}

	Task<string> CreateSupplierAsync(SapSupplierCreateRequest request, CancellationToken cancellationToken = default)
	{
		throw new NotSupportedException("Die SAP-Lieferantenanlage ist nicht verfügbar.");
	}

	Task<SapPostingResult> CreatePurchaseInvoiceAsync(SapPurchaseInvoiceRequest request, string actor, CancellationToken cancellationToken = default)
	{
		throw new NotSupportedException("Die SAP-Eingangsrechnungsanlage ist nicht verfügbar.");
	}

	Task<SapPostingResult?> FindPurchaseInvoiceByProposalAsync(Guid proposalId, CancellationToken cancellationToken = default)
	{
		return Task.FromResult<SapPostingResult?>(null);
	}
